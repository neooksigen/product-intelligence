...
def search_answer_gpt(state: SearchState):
    response = client_openai.responses.create(
        model="gpt-5.4", #16 may 2026: try to use more sophisticated model for web search + parsing. 18 Jul 2026 : use gpt-5.6 for better results. Use back 5.4 again due 5.6 is too expensive...
        #model="gpt-5.4-mini",
        ...
        input=search_instructions_gpt.format(search_query=state['question'],country=state['country'])
    )
